"""
第5段階実験: Delta-l 掃引（壁 z=0 あり, Blakelet）/ Experiment 05 sweep.

目的:
    Blakelet 2スライダーモデルで位相差 Delta と距離 l を掃引し、
    平均流量 Q(Delta, l) を計算して可視化する（論文第4章数値解析の再現）。

実行例:
    python experiments/exp05_sweep_delta_l_wall.py
    python experiments/exp05_sweep_delta_l_wall.py --mode fine --workers 4

IDE ▷ 実行:
    config/default_params.py の EXP05_DEFAULT_MODE を "fast" または "fine" に設定する。
...
EXP_NAME = "exp05_sweep_delta_l_wall"
...
def _run_one_case(case: _Exp05Case) -> tuple[int, int, float]:
...
def run_experiment(
    *,
    ...
    """
    Delta-l 掃引（Blakelet）を実行し、CSV と図を出力する。
    ...
    sweep_defaults, solver_config = resolve_exp05_config(mode)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="exp05: Delta-l sweep for two-slider model (wall, Blakelet).",
    )
    parser.add_argument(
        "--mode",
        choices=("fast", "fine"),
        default=EXP05_DEFAULT_MODE,
        help=(
            "fast: coarse Delta grid + Euler. fine: dense grid + RK45. "
            f"Default from config: EXP05_DEFAULT_MODE={EXP05_DEFAULT_MODE!r}."
        ),
    )
    parser.add_argument(
        "--workers",
...
def main() -> None:
    ...
    logging.getLogger(__name__).info(
        "Starting exp05 sweep: mode=%s, workers=%d",
        args.mode,
        workers,
    )
