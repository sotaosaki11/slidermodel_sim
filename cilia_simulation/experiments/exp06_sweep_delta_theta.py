"""
第6段階実験: Delta-theta 掃引（l 固定、x-y 配置角）/ Experiment 06 sweep.

目的:
    Blakelet 2スライダーモデルで中心間距離 l* 固定のもと、
    x-y 平面内の相対配置角 theta を変えながら位相差 Delta を掃引し、
    平均流量 Q(Delta, theta) を計算して可視化する。
...
from core.utils import (
    PlotStyle,
    make_run_directory,
    plot_delta_opt_vs_theta,
    plot_q_heatmap_delta_theta,
    plot_q_vs_delta_multi_theta_from_q_map,
    save_parameters,
    save_summary,
)
...
def _run_one_case(case: _Exp06Case) -> tuple[int, int, float]:
    ...
    return case.i_theta, case.i_d, Q
...
def run_experiment(
    *,
    ...
    """
    Delta-theta 掃引（Blakelet, l 固定）を実行し、CSV と図を出力する。
    ...
    theta_values : ndarray, optional
        配置角 [rad]。省略時は EXP06_THETA_VALUES（45°, 90°）。
    """
    if workers is None:
    ...
    q_max_values = q_map[np.arange(theta_values.size), delta_opt_idx]
    ...
    save_parameters(
        run_dir / "parameters.json",
        {
            ...
            "l_star": l,
            "theta_values_rad": theta_values.tolist(),
            "theta_values_deg": np.degrees(theta_values).tolist(),
    ...
    for i_theta, (theta_rad, theta_deg) in enumerate(
        zip(theta_values, theta_values_deg, strict=True)
    ):
        for i_d, (delta_rad, delta_deg) in enumerate(
            ...
            rows.append(
                (
                    ...
                    float(delta_rad),
                    float(delta_deg),
                    float(q_map[i_theta, i_d]),
                )
            )
    np.savetxt(
        run_dir / "q_delta_theta.csv",
        np.asarray(rows, dtype=np.float64),
        delimiter=",",
        header="theta_rad,theta_deg,delta_rad,delta_deg,Q",
        comments="",
        encoding="utf-8",
    )

    np.savetxt(
        run_dir / "delta_opt_vs_theta.csv",
        np.column_stack(
            [theta_values, theta_values_deg, delta_opt_values, delta_opt_values_deg, q_max_values]
        ),
        delimiter=",",
        header="theta_rad,theta_deg,delta_opt_rad,delta_opt_deg,Q_max",
        comments="",
        encoding="utf-8",
    )

    plot_q_heatmap_delta_theta(
        run_dir / "Q_heatmap_delta_theta.png",
        delta_values,
        theta_values,
        q_map,
        l_fixed=l,
    ...
    plot_delta_opt_vs_theta(
        run_dir / "delta_opt_vs_theta.png",
        theta_values,
        delta_opt_values,
        l_fixed=l,
        style=PLOT_STYLE,
    ...
    plot_q_vs_delta_multi_theta_from_q_map(
        run_dir / "Q_vs_delta_multi_theta.png",
        theta_values=theta_values,
        delta_values=delta_values,
        q_map=q_map,
        l_fixed=l,
    ...
    global_i_theta, global_i_d = np.unravel_index(np.argmax(q_map), q_map.shape)
    summary_lines = [
        ...
        f"l_star: {l:.8e}",
        f"theta_values [deg]: {[f'{v:.4f}' for v in theta_values_deg]}",
        ...
        f"global_opt_theta [deg]: {float(theta_values_deg[global_i_theta]):.4f}",
        f"global_opt_delta [rad]: {float(delta_values[global_i_d]):.8e}",
        f"global_opt_delta [deg]: {float(delta_values_deg[global_i_d]):.4f}",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="exp06: Delta-theta sweep for two-slider model (wall, Blakelet, l fixed).",
    )
    parser.add_argument(
        "--mode",
        choices=("fast", "fine"),
...
def main() -> None:
    ...
    logging.getLogger(__name__).info(
        "Starting exp06 sweep: mode=%s, workers=%d, l*=%.4f, theta=%s",
        args.mode,
        workers,
        EXP06_L_STAR,
        [f"{math.degrees(t):.1f}°" for t in EXP06_THETA_VALUES],
    )
